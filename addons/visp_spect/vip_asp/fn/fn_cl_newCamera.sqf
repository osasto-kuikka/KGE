/*
	...
	Parameters (Init):
		...
		2: Array (optional); PosASL for initial camera position (by default, camera spawns next to player; with this, you can make a swanky establishing shot)
		
	Example:
		//camera isn't permanent, tracking is on while camera is on, with custom establishing shot positiion
		["Init", [false, 1, [3200, 1200, 50]]] call vip_asp_fnc_cl_newCamera;
...
switch _mode do {
	...
	case "Init": {
		...
		if (isClass (configFile >> "CfgPatches" >> "uo_main")) then {
			_uohud = a3_nameHUD_enabled;
			vip_asp_var_cl_UOHUD = _uohud;
			a3_nameHUD_enabled = false
		};
		...
		//apply marker settings for units already dead
		{
			...
			["UnitKilled", [_x, objNull]] call vip_asp_fnc_cl_newCamera;
		} forEach allDead;
	...
	case "Mouse": {
		...
		_camRotate = {
			if ((vip_asp_var_cl_lock select 0) > -1) exitWith {};
			_dX = (_this select 0) * vip_asp_var_cl_fov;
			_dY = (_this select 1) * vip_asp_var_cl_fov;
