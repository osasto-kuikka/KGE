...
if (_newMode == 0) then { // Free
    ...
    if (isNull GVAR(camAgent)) then {
        GVAR(camAgent) = createAgent ["VirtualMan_F",getpos KGE_Player,[],0,""];
    };

    // Set free camera above last spectated unit
    if !(isNull GVAR(camPrev)) then {
        private "_pos";
        _pos = getPos GVAR(camPrev);
        _pos set [2, 5];
        GVAR(camAgent) setPos _pos;
