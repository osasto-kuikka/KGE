...
// Edited version for A3 by nikolauska
//
...
class KEGsRscSpectate {
	...
	class controls {
		class mouseHandler: KEGsRscControlsGroup {
			...
			onMouseHolding = QUOTE([ARR_2(QUOTE(Mouse), _this)] call FUNC(eventHandler));
			onMouseMoving = QUOTE([ARR_2(QUOTE(Mouse), _this)] call FUNC(eventHandler));
			onMouseButtonDown = QUOTE([ARR_2(QUOTE(MouseButtonDown), _this)] call FUNC(eventHandler));
			onMouseButtonUp = QUOTE([ARR_2(QUOTE(MouseButtonUp), _this)] call FUNC(eventHandler));
			onMouseZChanged = QUOTE([ARR_2(QUOTE(MouseZChanged), _this)] call FUNC(eventHandler));
			onMapClick = QUOTE([ARR_2(QUOTE(MapClick), _this)] call FUNC(eventHandler));
			idc = 123;
		...
		class menuCamerasLB : KEGsRscListBox {
			...
			sizeEx = 0.025;
			onMouseButtonUp = QUOTE(call FUNC(cameraMenuHandler));
		};
		...
		class menuTargetsLB : KEGsRscListBox {
			...
			sizeEx = 0.025;
			period = 0;
			onMouseButtonUp = QUOTE(true call FUNC(cameraMenuHandler));
		};
		...
		class tCamera : KEGsRscActiveText {
			...
			shadow = true;
			font = "PuristaMedium";
			onMouseButtonUp = QUOTE([ARR_2(QUOTE(ToggleCameraMenu), 0)] call FUNC(eventHandler));
		};
		class tTarget : tCamera {
			idc = IDC_TARGET;
			text = "Target";
			style = 1;
			onMouseButtonUp = QUOTE([ARR_2(QUOTE(ToggleTargetMenu), 0)] call FUNC(eventHandler));
		};
		...
		class tHelp : KEGsRscText {
			type = 13;
			idc = IDC_HELP;
			x = SafeZoneX + BORDERXSIZE*3;
			y=SafeZoneY + BORDERSIZE*3; 
			w = SafeZoneW-(2*BORDERXSIZE*3); 
			h = SafeZoneH-(2*BORDERSIZE*2);
		...
		class map : KEGsRscMapControl {
			...
			idc = IDC_MAP;
			x = SafeZoneX + SafeZoneW-MAPWIDTH; 
			y = SafeZoneY + SafeZoneH-MAPHEIGHT;
			w = MAPWIDTH; 
			h = MAPHEIGHT;
			...
			onMouseZChanged = QUOTE([ARR_2(QUOTE(MouseZChangedminimap), _this)] call FUNC(eventHandler));
		...
		class mapFullEventLog : KEGsRscListBox {
			...
			idc = IDC_EVENTLOG;
			x = SafeZoneX; 
			y = SafeZoneY + SafeZoneH-ELOGHEIGHT;
			w = ELOGWIDTH;		
			h = ELOGHEIGHT;
			colorText[] = {1, 1, 1, 0};
			colorSelect[] = {1, 1, 1, 0};
