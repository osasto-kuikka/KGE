...
def main():
    ...
    print("""
  ######################################
  # KGE Development Environment Setup #
  ######################################

  This script will create your KGE dev environment for you.
  ...
  This script will create two hard links on your system, both pointing to your ACE3 project folder:
    [Arma 3 installation directory]\\{} => KGE project folder
    P:\\{}                              => KGE project folder
  ...
  It will also copy the required CBA includes to {}, if you do not have the CBA source code already.""".format(FULLDIR,FULLDIR,CBA))
    ...
    ppath = "P:\\{}\\{}".format(MAINDIR,PROJECTDIR)
    ...
    if os.path.exists("P:\\{}\\{}".format(MAINDIR,PROJECTDIR)):
        ...
        try:
            ...
            if platform.win32_ver()[0] == "7":
                subprocess.call(["cmd", "/c", "mklink", "/D", ppath, projectpath])
            else:
                subprocess.call(["cmd", "/c", "mklink", "/D", "/J", ppath, projectpath])
        except:
            raise
            print("Something went wrong during the link creation between {} ==> {}".format(ppath, projectpath))
            return 4
    ...
    if os.path.exists(os.path.join(armapath, MAINDIR, PROJECTDIR)):
        ...
        try:
            ...
            raise
            print("Something went wrong during the link creation between {} ==> {}".format(os.path.join(armapath, MAINDIR, PROJECTDIR), projectpath))
            return 5
